...
async def stream_reasoning() -> None:
    ...
    query = "最新のMicrosoft Learnのコンテンツで、FoundryのMCPツールを使ったAgentの例はありますか？"
    query = "Python と Rust の違いをweb検索して、Python から Rust を学ぶためのステップを教えてください。"
    ...
    async for chunk in agent.run(query, stream=True):
        for content in chunk.contents:
            if content.type == "function_call":
                print_red(content.arguments, end="", flush=True)
            if content.type == "function_result":
                print_red(content.result, end="", flush=True)
            if content.type == "mcp_server_tool_call":
                print_yellow(content.arguments, end="", flush=True)
            if content.type == "mcp_server_tool_result":
                print_yellow(content.output[0].text, end="", flush=True)
